...
class BattleHud extends FlxTypedGroup<FlxSprite> {
	...
	public function new() {
		...
		hpBar1 = new ProgressBar(x + 10, y + 15, 40, 4, 0, 100);
		for (member in hpBar1.members)
			add(member);

		hpBar2 = new ProgressBar(x + 80, y + 15, 40, 4, 0, 100);
	...
	public function loadBackground(path: String, width: Int, height: Int) {
		...
		marginTop = ViewPort.height - height;
	...
	private function updatePos() {
		...
		marginTop = cameraY + ViewPort.height - height;
